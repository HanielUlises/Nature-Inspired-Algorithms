...
int main() {
    ...
    for (size_t i = 0; i < objectiveFunctions.size(); ++i) {
        ...
        for (size_t j = 0; j < 20; j++){
            PSO pso(swarm_size, dimensions, objectiveFunctions[i], max_iterations, threshold);
    ...
    swarm_size = 100;  // Particles
    max_iterations = 100;
    omega_start = 0.9, omega_end = 0.4;
    PSO pso(swarm_size, dimensions, objectiveFunctions[0], max_iterations, threshold); // example with the first function
